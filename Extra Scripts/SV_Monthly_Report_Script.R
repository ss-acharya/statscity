...
for (i in 1:length(prot_split)) {
  ...
  prot_split[[i]] <- prot_split[[i]] %>% 
    select(ANALYTE, Colname)

  prot_join <- plyr::join_all(prot_split, type="full" ) #Join processed columns during looping
}
...
prot_join <- prot_join %>% 
  ...
  arrange(ANALYTE) %>% 
  mutate(., ANALYTE = recode(ANALYTE, 'Total Giardia Cyst Count' = 'Total Giardia IFA Cyst Count',
                              'Giardia EG' = 'Empty Giardia Cyst Count',
                              'Giardia AG' = 'Giardia Cyst w/ Amorphous Structure',
                              'Giardia GW1S' = 'Giardia Cyst w/ 1 Internal Structure',
                              'Giardia GW2S' = 'Giardia Cyst w/ >1 Internal Structure',
                              'Total Crypto Oocyst Count' = 'Total Crypto IFA Oocyst Count',
                              'Crypto EC' = 'Empty Crypto Oocyst Count',
                              'Crypto AC' = 'Crypto Oocyst w/ Amorphous Structure',
                              'Crypto CWS' = 'Crypto Oocyst w/ Internal Structure'))
...
prot_flex <- prot_join %>% 
  ...
  flextable(.) %>% 
  border_remove(.) %>% 
  add_header_row(., top = FALSE, values = c("Date Sampled:", date_headers)) %>% 
...
#############################
  ...
  doc <- read_docx("Protozoa_Report_Template.docx")
  doc <- body_add_flextable(doc, value = prot_flex, split = TRUE, pos = "after") %>% 
    body_add_par("") %>%
    body_add_par("") %>%
...
write.csv(final_coliqt_rep, csv_path, row.names=FALSE)
write.xlsx(final_coliqt_rep, report_path, row.names=FALSE)
